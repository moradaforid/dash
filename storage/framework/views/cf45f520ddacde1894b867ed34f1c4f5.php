<!-- New Table -->
<div x-show="!$store.<?php echo e($name); ?>.loading && !$store.<?php echo e($name); ?>.noRecordsFound" class="w-full border border-gray-200 dark:border-gray-700 overflow-hidden rounded-lg shadow-xs">
    <div class="w-full overflow-x-auto">
        <table class="w-full whitespace-no-wrap">
            <thead>
                <tr class="text-xs font-semibold tracking-wide text-left text-gray-500 uppercase border-b dark:border-gray-700 bg-gray-50 dark:text-gray-400 dark:bg-gray-800">
                    <template x-for="column in <?php echo e($columns); ?>">
            ...
            <tbody class="bg-white divide-y dark:divide-gray-700 dark:bg-gray-800">
                ...
                <template x-for="row in $store.<?php echo e($name); ?>.all_data">
                    <tr class="text-gray-700 dark:text-gray-400">
                        <?php echo e($tableRows); ?>
    ...
    <div class="grid px-4 py-3 text-xs font-semibold tracking-wide text-gray-500 uppercase border-t dark:border-gray-700 bg-gray-50 sm:grid-cols-9 dark:text-gray-400 dark:bg-gray-800">
        <span class="flex items-center col-span-3" x-text="`Showing ${$store.<?php echo e($name); ?>.from}-${$store.<?php echo e($name); ?>.to} of ${$store.<?php echo e($name); ?>.total}`">
                ...
                <ul class="inline-flex items-center">
                    ...
                    <li>
                        ...
                        <div class="relative inline-flex self-center">
                            ...
                            <select x-model.number="$store.<?php echo e($name); ?>.per_page" x-init="$watch('$store.<?php echo e($name); ?>.per_page', value => $store.<?php echo e($name); ?>.reset())" class="text-xs rounded border border-gray-200 dark:border-gray-500 text-gray-500 dark:text-gray-400 bg-white dark:bg-gray-700 hover:border-gray-400 focus:outline-none appearance-none">
                    ...
                    <li>
                        <button :disabled="!$store.<?php echo e($name); ?>.pre_btn" @click="$store.<?php echo e($name); ?>.prev()" class="px-3 py-1 rounded-md rounded-l-lg focus:outline-none focus:shadow-outline-purple" aria-label="Previous">
                    ...
                    <li>
                        <button :disabled="!$store.<?php echo e($name); ?>.nex_btn" @click="$store.<?php echo e($name); ?>.next()" class="px-3 py-1 rounded-md rounded-r-lg focus:outline-none focus:shadow-outline-purple" aria-label="Next">
...
<!-- Loading Transition -->
<div x-show="$store.<?php echo e($name); ?>.loading" class="w-full border border-gray-200 dark:border-gray-700 bg-white dark:bg-gray-800 overflow-hidden rounded-lg shadow-xs p-4 mx-auto">
    <div class="animate-pulse flex space-x-4">
        ...
        <div class="flex-1 space-y-6 py-1">
            ...
            <template x-for="row in $store.<?php echo e($name); ?>.per_page * 2">
                <!-- row -->
                <div class="space-y-3">
...
<!-- No records found -->
<div x-show="$store.<?php echo e($name); ?>.noRecordsFound" class="w-full border border-gray-200 dark:border-gray-700 bg-white dark:bg-gray-800 overflow-hidden rounded-lg shadow-xs p-4 mx-auto" style="display: none;">
...
<div x-data="{singular_name : '<?php echo e($singularName); ?>'}" x-show="isModalOpen" x-transition:enter="transition ease-out duration-150" x-transition:enter-start="opacity-0" x-transition:enter-end="opacity-100" x-transition:leave="transition ease-in duration-150" x-transition:leave-start="opacity-100" x-transition:leave-end="opacity-0" class="fixed inset-0 z-50 flex items-end bg-black bg-opacity-50 sm:items-center sm:justify-center" style="display: none;">
    ...
    <div id="edit-modal" x-show="isModalOpen" @click.away="closeModal" @keydown.escape="closeModal" x-transition:enter="transition ease-out duration-150" x-transition:enter-start="opacity-0 transform translate-y-1/2" x-transition:enter-end="opacity-100" x-transition:leave="transition ease-in duration-150" x-transition:leave-start="opacity-100" x-transition:leave-end="opacity-0  transform translate-y-1/2" class="max-h-full overflow-y-auto w-full px-6 py-4 bg-gray-100 rounded-t-lg dark:bg-gray-800 sm:rounded-lg sm:m-4 sm:max-w-xl" role="dialog">
        ...
        <footer class="flex flex-col justify-end sm:flex-row sm:space-x-4 space-y-3 sm:space-y-0 mt-2">
            ...
            <!-- create button -->
            <button x-show="isCreateModalOpen" @click="createItem()" :disabled="<?php echo e($disabledSave); ?>" class="disabled:opacity-50 w-full px-5 py-3 text-sm font-medium leading-5 text-white transition-colors duration-150 bg-purple-600 border border-transparent rounded-lg sm:w-auto sm:px-4 sm:py-2 active:bg-purple-600 hover:bg-purple-700 focus:outline-none focus:shadow-outline-purple">
                Create
            </button>

            <!-- edit button -->
            <button x-show="isEditModalOpen" @click="editItem()" :disabled="<?php echo e($disabledSave); ?>" class="disabled:opacity-50 w-full px-5 py-3 text-sm font-medium leading-5 text-white transition-colors duration-150 bg-purple-600 border border-transparent rounded-lg sm:w-auto sm:px-4 sm:py-2 active:bg-purple-600 hover:bg-purple-700 focus:outline-none focus:shadow-outline-purple">
                Save
            </button>
...
<script>
    document.addEventListener('alpine:init', () => {
        ...
        Alpine.store('<?php echo e($name); ?>', {
            path: "<?php echo e($crudPath); ?>",
                ...
                fetch(this.crudPath + '/' + this.deleteId, {
                    ...
                    .then((data) => {
                        ...
                        if (data.status == 'deleted') {
                            Alpine.store('<?php echo e($name); ?>').reset()
                            this.closeModal()
                        } else {
...
</script><?php /**PATH C:\Users\21265\Desktop\Projects\Dash - No Vuejs\resources\views/components/table.blade.php ENDPATH**/ ?>