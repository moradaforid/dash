...
<?php $__env->startSection('pageTitle', isset($pageTitle) ? $pageTitle : 'Edit App'); ?>
        ...
        <div class="container-xl">
            <div class="row row-cards">
                <div class="col-lg-12">
                    <div class="row row-cards">
                        <div class="col-12">
                            <form action="/apps/<?php echo e($app->id); ?>" method="POST" class="card">
                                <?php echo csrf_field(); ?>
                                    ...
                                    <div class="row row-cards">
                                        ...
                                        <div class="col-sm-6 col-md-6">
                                            <div class="mb-3">
                                                ...
                                                <input type="text" class="form-control" name="name" placeholder="Name" value="<?php echo e($app->name); ?>">
                                            </div>
                                        </div>
                                        <div class="col-sm-6 col-md-6">
                                            <div class="mb-3">
                                                <label class="form-label">Category</label>
                                                <input type="text" class="form-control" name="category_id" placeholder="Category" value="<?php echo e($app->category_id); ?>">
                                            </div>
                                        </div>
...
<?php $component = Illuminate\View\AnonymousComponent::resolve(['view' => 'components.table','data' => ['name' => 'table_data','editType' => 'modal','path' => ''.e(route('category.index')).'']] + (isset($attributes) && $attributes instanceof Illuminate\View\ComponentAttributeBag ? (array) $attributes->getIterator() : [])); ?>
...
<?php endif; ?>
<?php $component->withAttributes(['name' => 'table_data','editType' => 'modal','path' => ''.e(route('category.index')).'']); ?>
...
<?php endif; ?>
    ...
    <div x-show="isModalOpen" x-transition:enter="transition ease-out duration-150" x-transition:enter-start="opacity-0" x-transition:enter-end="opacity-100" x-transition:leave="transition ease-in duration-150" x-transition:leave-start="opacity-100" x-transition:leave-end="opacity-0" class="fixed inset-0 z-50 flex items-end bg-black bg-opacity-50 sm:items-center sm:justify-center" style="display: none;">
        ...
        <!-- Create Modal -->
        <div id="create-modal" x-show="isCreateModalOpen" @click.away="closeModal" @keydown.escape="closeModal" x-transition:enter="transition ease-out duration-150" x-transition:enter-start="opacity-0 transform translate-y-1/2" x-transition:enter-end="opacity-100" x-transition:leave="transition ease-in duration-150" x-transition:leave-start="opacity-100" x-transition:leave-end="opacity-0  transform translate-y-1/2" class="w-full px-6 py-4 overflow-hidden bg-white rounded-t-lg dark:bg-gray-800 sm:rounded-lg sm:m-4 sm:max-w-xl" role="dialog">
            <!-- Remove header if you don't want a close icon. Use modal body to place modal tile. -->
            <header class="flex justify-end">
                <button class="inline-flex items-center justify-center w-6 h-6 text-gray-400 transition-colors duration-150 rounded dark:hover:text-gray-200 hover: hover:text-gray-700" aria-label="close" @click="closeModal">
                    <svg class="w-4 h-4" fill="currentColor" viewBox="0 0 20 20" role="img" aria-hidden="true">
        ...
        <!-- Edit Modal -->
        <div id="edit-modal" x-show="isEditModalOpen" @click.away="closeModal" @keydown.escape="closeModal" x-transition:enter="transition ease-out duration-150" x-transition:enter-start="opacity-0 transform translate-y-1/2" x-transition:enter-end="opacity-100" x-transition:leave="transition ease-in duration-150" x-transition:leave-start="opacity-100" x-transition:leave-end="opacity-0  transform translate-y-1/2" class="w-full px-6 py-4 overflow-hidden bg-gray-100 rounded-t-lg dark:bg-gray-800 sm:rounded-lg sm:m-4 sm:max-w-xl" role="dialog">

            <!-- Remove header if you don't want a close icon. Use modal body to place modal tile. -->
            <header class="flex flex-row">
                <!-- Modal title -->
                <p class="flex-1 text-lg font-semibold text-gray-700 dark:text-gray-300">Edit Category</p>
            ...
            <footer class="flex flex-col justify-end sm:flex-row sm:space-x-4 space-y-3 sm:space-y-0">
                ...
                <button @click="editItem('/categories')" :disabled="!editRow.name" class="disabled:opacity-50 w-full px-5 py-3 text-sm font-medium leading-5 text-white transition-colors duration-150 bg-purple-600 border border-transparent rounded-lg sm:w-auto sm:px-4 sm:py-2 active:bg-purple-600 hover:bg-purple-700 focus:outline-none focus:shadow-outline-purple">
                    Save
                </button>
            </footer>
...
<?php echo $__env->make('layouts.dashboard', \Illuminate\Support\Arr::except(get_defined_vars(), ['__data', '__path']))->render(); ?><?php /**PATH C:\Users\21265\Desktop\Projects\laravel-vue\resources\views/app/edit.blade.php ENDPATH**/ ?>